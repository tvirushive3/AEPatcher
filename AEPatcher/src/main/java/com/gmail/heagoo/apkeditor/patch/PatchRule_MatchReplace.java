...
class PatchRule_MatchReplace extends PatchRule {
    ...
    private static final ThreadPoolExecutor PATCH_EXECUTOR = new ThreadPoolExecutor(THREADS, THREADS, 120, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
    ...
    public String executeRule(MainActivity activity, ZipFile patchZip, IPatchContext logger) {
        ...
        if (this.bRegex) {
            ...
            try {
                pattern = (bDotall ? Pattern.compile(regStr.trim(), Pattern.DOTALL) : Pattern.compile(regStr.trim()));
        ...
        while (nextPath != null) {
            final String targetFile = nextPath;
            futures.add(PATCH_EXECUTOR.submit(new Runnable() {
                @Override
                public void run() {
                    executeOnEntry(activity, patchZip, logger, targetFile, finalPattern);
                }
            }));
        ...
        for (Future<?> future : futures) {
            try {
                ...
                // executeOnEntry handles expected patch errors itself.
                // Keep waiting for the remaining tasks if one task fails unexpectedly.
            }
        }
        return null;
    }

    private void executeOnEntry(MainActivity activity, ZipFile patchZip, IPatchContext patchCtx, String targetFile, Pattern pattern) {
